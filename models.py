from . import Store, Field, BooleanField, DateField, IntegerField, FloatField, LongIntField, EmbeddedStoreField
...
class Document(Store, RpcMixin):
    ...
    class Category(Store):
        id = Field(target='ID')
        name = Field(target='name')
        offset = IntegerField(target='offset')
        count = IntegerField(target='count')
        subtree_size = IntegerField(target='subtreeSize')
    ...
    votes = IntegerField(target='numberOfVotes')
    ...
    version_size = IntegerField(target='versionSize')
